...
export function MateriePage() {
    ...
    const [activeDegreeId, setActiveDegreeId] = useState<number | null>(null);
    ...
    const tree = useMemo(() => {
        ...
        for (const c of courses) {
            ...
            if (!areaMap.has(degree.macroArea))
                areaMap.set(degree.macroArea, new Map());
            const area = areaMap.get(degree.macroArea)!;

            if (!area.has(groupKey))
                area.set(groupKey, [] as YearLeaf[]);
            const group = area.get(groupKey)!;

            const year = group.find((y) => y.degreeId === degree.id);
            if (year) {
                year.courses.push(c);
            } else {
                group.push({ degreeId: degree.id, degreeYear: degree.degreeYear, courses: [c] });
            }
        }
    ...
    const filteredYear = useMemo(() => {
        if (!searchTerm.trim() || !activeDegreeId) return undefined;
        for (const areaNode of tree) {
            for (const group of areaNode.groups) {
                const year = group.years.find((y) => y.degreeId === activeDegreeId);
                if (year) {
                    const term = searchTerm.toLowerCase();
                    return {
                        ...year,
        ...
        return undefined;
    }, [searchTerm, activeDegreeId, tree]);
        ...
        <div className="space-y-4">
            ...
            {!loading && !error && tree.length === 0 && (
                <div className="bg-white border border-line rounded-xl shadow-sm p-8 text-center">
                    <p className="text-ink-4">Nessuna materia inserita a sistema.</p>
                </div>
            )}
                                                    ...
                                                    {groupOpen && (
                                                        <div>
                                                            {group.years.map((year) => {
                                                                ...
                                                                const yearOpen = activeDegreeId === year.degreeId;
                                                                const displayCourses = yearKey === `year_${activeDegreeId}` && searchTerm
                                                                    ? filteredYear?.courses ?? year.courses
                                                                    : year.courses;
                                                                return (
                                                                    <div key={year.degreeId} className="border-b border-line/20 last:border-b-0">
                                                                        <button
                                                                            onClick={() => setActiveDegreeId(yearOpen ? null : year.degreeId)}
            ...
            {isModalOpen && (
                <MateriaModal
                    ...
                    defaultDegreeId={modalMode === 'create' ? activeDegreeId ?? undefined : undefined}
                    onClose={closeModal}
                    onSave={loadCourses}
                />
            )}
